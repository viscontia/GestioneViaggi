...
*   **Chip di stato del selettore edizione.** Due informazioni distinte, entrambe con tooltip: *"Con contenuto (stato)"* riguarda la **scheda web**; *"Partenza da effettuare / già effettuata"* riguarda la **partenza** e legge `ana_date_viaggi.data_viaggio_effettuato_sino` — la spunta "Viaggio Effettuato" della scheda Date del viaggio, la stessa che alimenta le statistiche viaggi fatti/da fare. Serve qui perché curare la scheda web di una partenza già avvenuta è quasi sempre lavoro sprecato. ⚠️ I colori erano invertiti (giallo su "Da effettuare", che è lo stato **normale** di una data futura): ora il richiamo è su "già effettuata".
