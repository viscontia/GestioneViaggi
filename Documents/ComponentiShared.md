...
*   **`StatoPartenzaChip`** (`Components/Shared/StatoPartenzaChip.razor`): chip riusabile con lo stato della partenza e il relativo tooltip. Parametri `Effettuato` (required) e `DataFine`. La sola resa grafica: regole e testi stanno in `StatoPartenzaRules`, così la stessa lettura vale ovunque si ragioni su una partenza (contenuti web, elenco date, calendario).
