...
## 0.3 Sito di iscrizione — chi è già cliente: usare, completare, modificare (`SqlScripts/660`, `661`)

Disegno: `Iscrizione-Viaggi-Offroad PostgreSQL/docs/plans/2026-09-25-cliente-riconosciuto-otp-design.md`.
Il cliente riconosciuto dall'email **usa** la scheda in archivio; **completa** senza codice solo
i campi vuoti; per **vedere o modificare** passa da un codice usa e getta mandato alla casella
**in archivio**.

⛔️ **Nessuna di queste funzioni è concessa ad `anon`/`authenticated`**: le chiama solo il sito
Flask, che si connette come `postgres`, proprietario. Tutte hanno `SET search_path = public, pg_temp`.

| Funzione | Cosa fa | Files Coinvolti |
|---|---|---|
| `fn_web_otp_genera(p_azienda_id, p_cliente_id)` → `TABLE(esito, codice, email)` | Codice a 8 cifre per la casella in archivio, restituito **una volta** per spedirlo; in tabella (`web_otp_codici`) solo sha256(sale‖codice). 5 minuti; un codice nuovo annulla i precedenti; al massimo 3 richieste ogni 15 minuti e 10 al giorno per cliente; lock per cliente contro le richieste parallele. Esiti: OK, CLIENTE_ASSENTE, SENZA_EMAIL, TROPPE_RICHIESTE, EMAIL_NON_VERIFICATA (dal 662) | `Classi_Tabelle_DB/cliente.py` (sito Flask) |
| `fn_web_otp_verifica(p_azienda_id, p_cliente_id, p_codice)` → `text` | Verifica l'ultimo codice aperto: OK (e non vale più), ERRATO, SCADUTO, TENTATIVI_ESAURITI (3), NESSUN_CODICE. ⚠️ Il chiamante deve fare **commit anche su ERRATO**, o i tentativi tornano indietro | idem |
| `fn_web_cliente_completa(p_cliente_id, p_azienda_id, p_dati)` → `TABLE(campo, etichetta)` | L'**unica** scrittura del sito su una scheda esistente senza codice: riempie solo i campi vuoti di una whitelist dei campi del modulo, mai email e consenso; valida **prima** di scrivere e risponde con testi fissi, **mai con dati della scheda** (il messaggio originale di `fn_cf_verifica` conteneva il codice fiscale atteso); codice fiscale e dati di nascita solo se si confermano con un codice fiscale già in archivio; rifiuta le schede senza email («Cliente non trovato.», come per chi non esiste). Restituisce le etichette per l'avviso al titolare | idem |
| `fn_web_cliente_completa_esito_cf(p_scheda jsonb)` → `varchar` | Solo il **codice esito** di `fn_cf_verifica` su una scheda, senza messaggio né codice fiscale atteso: serve a 661 | — |
| `fn_web_cliente_aggancia_email(p_cliente_id, p_azienda_id, p_email)` → `boolean` (`SqlScripts/662`) | Aggancia l'email a una scheda che non l'ha (chiama `fn_ana_clienti_aggancia_email`, 593) e **registra** in `web_email_agganciate` che l'ha agganciata il sito. È quella che il sito usa al posto del 593 | `Classi_Tabelle_DB/cliente.py` (sito Flask) |
| `fn_web_email_da_confermare(p_cliente_id, p_azienda_id)` → `boolean` (`SqlScripts/668`, L12) | True se l'email attuale della scheda è quella agganciata dal sito e nessuno l'ha confermata: la scheda cliente mostra l'avviso | `ClienteEmailWebService` (gestionale) |
...
⚠️ **Cambia una decisione del 2026-09-05** (script 594): sovrascrivere recapiti, indirizzo e
documento dal sito ora è una **modifica**, e richiede il codice.
...
**File Documentazione Completa**: `Documents/Storico/Changelog_ExchangeRateIntegration.md`
...
Fase 1 del piano `Documents/Analisi_e_Design/piani-storici/2026-08-20-ana-clienti-un-controllo-un-posto-solo.md`.
...
| `fn_get_bilancio_viaggio` | Recupera dati economici per report **Bilancio di Viaggio** (Analisi Margini). Classifica le transazioni in **RICAVI** (ciclo attivo) e **COSTI** (ciclo passivo) e calcola importi normalizzati (imponibile, IVA, lordo) gestendo i segni delle causali. ⭐️ **`importo_effettivo_eur` è il valore su cui si fa il margine**: il netto, tranne sui COSTI di un'azienda con `is_iva_detraibile = FALSE` (forfettario), dove è il lordo — quell'IVA non si recupera ed è costo (script 638). | `p_azienda_id INT, p_viaggio_id INT, p_data_viaggio_id INT, p_data_da DATE, p_data_a DATE` | `TABLE(..., importo_netto_eur, importo_iva_eur, importo_lordo_eur, importo_effettivo_eur, importo_pagato_eur, stato_pagamento)` | `Services/Printing/BilancioViaggioPrintService.cs`, `SqlScripts/638_Bilancio_Viaggi_IvaNonDetraibile.sql` |
| `fn_get_bilancio_annuale_viaggi` | Estrae i movimenti contabili (attivi e passivi) dell'anno di competenza, raggruppati per data viaggio, per il report a 3 livelli (totale data, totale viaggio, riepilogo). ⭐️ Espone `importo_effettivo_eur` con la stessa regola di `fn_get_bilancio_viaggio` (script 638). | `p_azienda_id INT, p_anno INT` | `TABLE(..., importo_netto_eur, importo_iva_eur, importo_lordo_eur, importo_effettivo_eur, importo_pagato_eur, stato_pagamento)` | `Services/Printing/BilancioViaggioPrintService.cs`, `SqlScripts/638_Bilancio_Viaggi_IvaNonDetraibile.sql` |
...
| Nome della Function | Scopo | Input | Output | Files Coinvolti |
| :--- | :--- | :--- | :--- | :--- |
| `fn_wizard_get_client_data` | Recupera i dati anagrafici di un cliente per la validazione del codice fiscale. LEFT JOIN tra `ana_clienti` e `ana_geo_comuni` per ottenere il codice catastale del comune di nascita. | `p_cliente_id INTEGER, p_azienda_id INTEGER` | `TABLE(cliente_cognome, cliente_nome, cliente_data_nascita, cliente_sesso, comune_codfisc, cliente_intolleranza)` | `Classi_Tabelle_DB/cliente.py` (`get_client_data()`). ⛔️ Dal `SqlScripts/663` ha un secondo parametro **`p_azienda_id`** e legge solo i clienti di quell'azienda; la firma senza azienda la cancella il `667`, dopo il rilascio del sito nuovo. |
...
| `fn_wizard_get_partecipanti_details` | Recupera dati anagrafici completi per un array di ID cliente. Usa `WHERE cliente_id = ANY(p_ids)`. Chiamato con cast esplicito `::integer[]`. Usato nel riepilogo Step 5 e nell'email di conferma. ⛔️ Dal `SqlScripts/663` ha un secondo parametro **`p_azienda_id`** e legge solo i clienti di quell'azienda; la firma senza azienda la cancella il `667`, dopo il rilascio del sito nuovo. | `p_ids INTEGER[], p_azienda_id INTEGER` | `TABLE(cliente_id, cliente_cognome, cliente_nome, cliente_email, cliente_data_nascita, cliente_intolleranza)` | `Classi_Tabelle_DB/cliente.py` (`get_partecipanti_details()`) |
...
| Nome della Function | Scopo | Input | Output | Files Coinvolti |
| :--- | :--- | :--- | :--- | :--- |
| `fn_wizard_get_partecipanti` | Recupera dati anagrafici per un array di ID cliente. Usa `WHERE cliente_id = ANY(p_ids)`. Usato per visualizzare la lista passeggeri selezionati. ⛔️ Dal `SqlScripts/663` ha un secondo parametro **`p_azienda_id`** e legge solo i clienti di quell'azienda; la firma senza azienda la cancella il `667`, dopo il rilascio del sito nuovo. | `p_ids INT[], p_azienda_id INT` | `TABLE(cliente_id, cliente_cognome, cliente_nome, cliente_email, cliente_data_nascita)` | `app.py` (endpoint `/api/partecipanti`) |
...
| `fn_ana_tipo_alloggio_get_all` |  | TABLE(tipo_alloggio_id integer, tipo_alloggio_descrizione character varying, tipo_alloggio_numero_occupanti integer, tipo_alloggio_supplemento character varying, genere_fk integer, genere_descrizione character varying, genere_codice character varying, tipo_alloggio_mai_proposta boolean) |  |
| `fn_ana_tipo_alloggio_upsert` | p_id integer, p_descrizione character varying, p_numero_occupanti integer, p_supplemento character varying, p_genere_fk integer, p_mai_proposta boolean DEFAULT false | integer |  |
...
| `fn_ana_tipo_pernottamento_generi_get` | p_pernottamento_id integer | TABLE(genere_id integer, codice character varying, descrizione character varying, ammesso boolean, di_sistema boolean) |  |
| `fn_ana_tipo_pernottamento_generi_set` | p_pernottamento_id integer, p_generi integer[] | integer | Imposta i generi ammessi da un pernottamento. ⛔️ **Rifiuta** — non chiede — se il cambio |
...
| `fn_get_registro_iva_print_data` | p_azienda_id integer, p_periodo_da date, p_periodo_a date | jsonb |  |
...
| `fn_guardia_alloggio_coerente` |  | trigger | Guardia su mov_clienti_alloggi: il genere dev'essere previsto dal viaggio e la capienza |
...
| `fn_mov_clienti_viaggi_cancellazione_camere` | p_data_viaggio_id integer, p_cliente_id integer | TABLE(alloggio_pk integer, tipo_descrizione character varying, chi_esce text, restano text, quanti_restano integer) | Le sistemazioni che resterebbero con meno persone cancellando questo partecipante (e, se |
...
| `fn_mov_clienti_viaggi_delete` | p_viaggio_id integer, p_data_viaggio_id integer, p_cliente_id integer, p_adeguamenti jsonb DEFAULT '[]'::jsonb | integer |  |
...
| `get_rooms_with_occupants` | p_data_viaggio_id integer | TABLE(alloggio_pk integer, tipo_alloggio text, tipo_alloggio_id integer, max_occupants integer, current_occupants integer, occupant_names text[], occupant_ids integer[], has_supplement boolean, pilot_cognome text) |  |
...
- `fn_alloggi_salva_camera`
- `fn_alloggi_tipi_ammessi`
- `fn_alloggi_tipi_per_capienza`
- `fn_alloggi_tipo_predefinito`
