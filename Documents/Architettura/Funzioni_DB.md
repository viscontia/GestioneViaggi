...
|---|---|---|
| `fn_web_otp_genera(p_azienda_id, p_cliente_id)` → `TABLE(esito, codice, email)` | Codice a 8 cifre per la casella in archivio, restituito **una volta** per spedirlo; in tabella (`web_otp_codici`) solo sha256(sale‖codice). 5 minuti; un codice nuovo annulla i precedenti; al massimo 3 richieste ogni 15 minuti e 10 al giorno per cliente; lock per cliente contro le richieste parallele. Esiti: OK, CLIENTE_ASSENTE, SENZA_EMAIL, TROPPE_RICHIESTE | `Classi_Tabelle_DB/cliente.py` (sito Flask) |
