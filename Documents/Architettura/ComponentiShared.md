...
*   **Usato da**: `ViaggioDatesManager` (colonna EFFETT.), `WebEdizioniManager` (selettore edizione), `TravelDataSelectorDialog` (albero «Per Anno» **e** tendina delle date), `TravelCalendar` (colore delle targhette) e `TravelTooltipContent`. Dettagli nella sezione contenuti web.
*   ⚠️ **È l'unico vocabolario rimasto.** Esisteva in parallelo un `TravelStatus` che incrociava il flag con la data di **inizio** anziché di fine, con tre esiti colorati in modo fuorviante: **arancione a ogni partenza futura** — il caso normale segnalato come allarme — e **rosso** a quelle passate. Viveva nel selettore, nel calendario e nel suo tooltip; è stato rimosso del tutto. Se serve mostrare lo stato di una partenza, si usa questo componente, oppure `StatoPartenzaRules.Valuta` quando serve solo il colore (il calendario ha celle troppo strette per un chip e mappa il `Livello` sui suoi colori: neutro in programma, blu conclusa, ambra anomalia).
...
*   **Avviso VPN**: testo in dialog che avvisa che il test connessione può fallire con VPN attiva (molti server di posta bloccano gli IP VPN/datacenter); i messaggi d'errore del test sono generati da `SmtpErrorTranslator` (vedi `Documents/Architettura/Gestione_check.md` §SmtpErrorTranslator), inclusa la diagnostica che distingue "host irraggiungibile" da "porta bloccata da firewall/VPN".
