...
| M6 | Nella casella di collaudo, le mail «**Abbiamo completato la tua scheda**» del 2026-09-26 alle 09:17, 09:20, 09:22 e 09:23 (le hanno mandate H33, H2, H3b e H4) | Ognuna elenca **quali campi** sono stati aggiunti, **senza i valori**: prefisso e telefono (09:17 e 09:20), data di nascita (09:22), codice fiscale (09:23). Il mittente è l'utenza SMTP del sito; «Rispondi» va all'email principale dell'azienda. ⛔️ Quindi **non rispondere**: arriverebbe all'azienda vera | ✅ 2026-09-26, con riserva. Le 4 mail del mattino **non sono mai arrivate** a Gmail, pur accettate dal server SMTP senza errori. Diagnosi con tre invii di prova: arriva la mail senza `Reply-To` e quella con `Reply-To: segreteria@sardegnafuoritraccia.it` (stesso dominio del mittente); **sparisce** quella con `Reply-To` su un indirizzo Gmail, cioè l'email aziendale **anonimizzata** del DB locale (`mirania008+…@gmail.com`). Il contenuto è giusto: elenca i campi aggiunti, senza valori. In PROD l'azienda 2 ha `segreteria@sardegnafuoritraccia.it` (caso che arriva); ⚠️ l'azienda 6 ha un indirizzo Gmail come email principale: da verificare con il suo SMTP prima di usare il sito per lei |
| M7 | H11b: database fermo quando il passo 2 legge il profilo | Vedi H11b | ✅ 2026-09-26, dopo due correzioni: col database che non risponde (`docker pause`) il wizard restava sullo spinner per sempre con l'icona verde, e al passo 1 diceva «email non valida». Ora ogni chiamata si interrompe dopo 8 secondi, l'icona diventa rossa e sotto il campo email compare un riquadro fisso «Il sistema di iscrizione non risponde…», che sparisce quando il database torna (`b2aac301`, `e87b0db4`, `a463f975`). Il caso «database che cade proprio mentre il passo 2 legge il profilo» non si riproduce a mano: coperto dalla prova simulata del 503 |
| M8 | *(aggiunta il 2026-09-26, decisione I4)* Scheda **senza email** in archivio: dal gestionale crea un cliente di prova senza email, con documento **scaduto** e viaggio all'estero. Sul sito, passo 1 con un'email nuova, passo 2 compilato da nuovo cliente con **gli stessi** cognome, nome, data e comune di nascita, «Avanti» | Nessun «Nuovo cliente inserito»: compare «**Eri già in archivio: useremo la tua scheda**…» e subito dopo un errore che resta a video con il motivo (documento scaduto) e «I dati in archivio li può aggiornare solo l'organizzazione: scrivi a …». Non si passa al passo 3. Poi cancellare il cliente di prova dal gestionale | Da eseguire a mano |
| M9 | *(2026-09-26, decisione I5)* Con la 3870, se ha già risposto alla domanda sulle comunicazioni: in DevTools, Console, `fetch('/api/cliente/consenso/risposta',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({cliente_id:3870,risposta:false})}).then(r=>r.status)` dopo il riconoscimento, **senza** codice | `403`: senza codice il consenso non si revoca. Con `risposta:true` risponde `200` | ✅ 2026-09-26 con `curl` sul sito locale: 3870 riconosciuta, `da_chiedere: false`, il NO senza codice risponde **403** «Per cambiare il consenso serve il codice.»; il SI' senza codice e il NO con il codice sono coperti dai test pytest (non ripetuti qui per non riscrivere il consenso della 3870) |
| M10 | *(2026-09-26, C1)* Viaggio con albergo, al passo 5 in Console: `fetch('/api/session/assegna-alloggio',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({tipo_alloggio_id:1,partecipanti_ids:[964]})}).then(r=>r.status)` | `403`: in camera solo i partecipanti di questa iscrizione. L'assegnazione normale dal passo 5 funziona come prima | ✅ 2026-09-26 con `curl` sul sito locale: pilota 3870 in sessione, `assegna-alloggio` con [3870, 964] → **403**, con [3870] → 200; `alloggi/valida` con un estraneo → **403** |
| M11 | *(L12-bis)* Scheda senza codice possibile (email agganciata dal sito, o nessuna email) e documento scaduto: passo 2 → nel riquadro **«Proponi i dati aggiornati»** al posto di «Modifica i miei dati» → finestra con i campi **vuoti** → nuova scadenza → «Invia all'organizzazione» | «Grazie: l'organizzazione controllerà i dati. Ti scriveremo a …»; nel database una proposta `IN_ATTESA` e la scheda **invariata**; alla segreteria la mail «Correzione da approvare», con i nomi dei campi e senza valori. ✅ 2026-09-26 in automatico sul sito locale (3870 con email resa agganciata): resta da vedere la mail | Da eseguire a mano |
| M12 | *(L12-bis)* Gestionale 2.3, scheda cliente della M11 | Riquadro «Correzione proposta dal sito» con *campo · in archivio · proposto*. **Approva** → la scheda si chiude; riaperta ha i dati nuovi (firmati dall'utente del gestionale) e al cliente arriva «La tua scheda è aggiornata: puoi completare l'iscrizione» col link al sito. **Scarta** (su un'altra proposta) → scheda invariata, nessuna mail. Una proposta con la scadenza prima del rilascio → «Non approvata: …», niente cambia | Da eseguire a mano. ⚠️ In locale c'è già una proposta sulla 3870 (telefono 3339990001, scadenza 31/01/2036), lasciata apposta |
