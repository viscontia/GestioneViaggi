...
- **Nuova funzione** `fn_ana_aziende_smtp_secrets_get(p_smtp_id uuid, p_master text)` → ritorna `(password text, inbound_password text)` via `pgp_sym_decrypt(password_enc, p_master)` / `pgp_sym_decrypt(inbound_password_enc, p_master)` (NULL-safe su colonne NULL). Script `SqlScripts/NNN_*.sql` + aggiornare `Documents/Architettura/Funzioni_DB.md`.
...
- `SqlScripts/NNN_Smtp_Secrets_Get.sql` (nuova funzione) + `Documents/Architettura/Funzioni_DB.md`
...
- `Documents/Architettura/ComponentiShared.md` / `Documents/Architettura/Gestione_check.md` (documentare il nuovo translator come punto centralizzato)
