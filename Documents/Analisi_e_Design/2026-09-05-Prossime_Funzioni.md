...
`Documents/Progetti/Estensione_Web/2026-09-05-Analisi_Protezione_Dati_Personali_Sito.md`.
...
| Cosa | Perché aspetta |
|---|---|
| **Passaggio a MudBlazor 9** | Risolverebbe alla radice i difetti #9090 e #11217 della libreria, aggirati a mano nei campi data (vedi `Documents/Architettura/Digitazione_Date.md`). È un cambio di versione maggiore: **dopo** il go-live, mai durante un collaudo. |
| **Componente condiviso `CampoData`** | I campi data ripetono in 33 punti picker + convertitore + classe CSS + `TextUpdateSuppression`. Incapsularli in un componente solo è quanto chiede `ComponentiShared.md`, e farebbe sparire la soppressione di MUD0002 dal `.csproj` (vedi `Documents/Architettura/Digitazione_Date.md`). Rimandato: tocca 33 punti di chiamata, non durante un collaudo. |
...
| **Analisi iscrizioni e assegnazione camere** | Controlli probabilmente doppi e divergenti fra MAUI e Flask, come lo erano per `ana_clienti`. Stesso metodo: misurare su PROD prima di scrivere. Vedi `Documents/Analisi_e_Design/2026-08-20-Analisi_Validazioni_e_CRUD_AnaClienti.md`. |
...
## 10. Togliere l'import da Oracle dal programma — **deciso il 2026-09-09**

⛔️ **Il database Oracle non esiste più.** È stato spento e rimosso dal server il giorno del
go-live, e la sua password di ADMIN era comunque scaduta dal 2 giugno: era morto da tre mesi.

⚠️ **Nel gestionale l'import è però ancora tutto lì**, e quindi anche nella copia consegnata ad
Antonio: sette servizi registrati in `MauiProgram.cs` e una pagina `Components/Pages/Tools/
OracleImport.razor`. Chi la aprisse otterrebbe un errore di connessione a un server che non
risponde più.

ℹ️ La pagina **non è raggiungibile dal menu** — ci si arriva solo digitando l'indirizzo — e questo
è il motivo per cui la cosa è passata inosservata fino al riordino.

### Cosa va tolto

| | |
|---|---|
| Pagina | `Components/Pages/Tools/OracleImport.razor` |
| Servizi | i 7 in `Migrazione_Dati_Oracle/` (5 Oracle + 2 Excel) |
| Registrazioni | le voci in `MauiProgram.cs` |
| Pacchetto NuGet | il client Oracle, se non serve ad altro — ⚠️ **da verificare prima**: è il pezzo che alleggerisce davvero l'eseguibile |

⚠️ **I due servizi Excel non sono Oracle** (`ExcelAnaMezziImportService`,
`ExcelAnaMezziModelliImportService`): leggono i file di `inp_Excel/`. Vanno guardati a parte —
potrebbero servire ancora per caricare modelli di mezzi.

⚠️ **Prima di togliere, verificare che i dati importati siano tutti arrivati**: una volta rimosso
il codice, se emergesse un buco nella migrazione non ci sarebbe più modo di rifarla. La verifica
è già stata fatta il giorno del go-live (nessun dato da recuperare, Oracle era vuoto da mesi), ma
