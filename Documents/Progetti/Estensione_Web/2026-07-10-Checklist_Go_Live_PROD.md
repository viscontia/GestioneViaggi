...
> La verità sulle *funzioni* DB resta `Documents/Architettura/Funzioni_DB.md` (rigenerato da `deploy_sql.sh`). Questo documento traccia il **deploy**, non la definizione.
...
1. **Elimina tre firme superate** di `fn_get_bilancio_viaggio` / `fn_get_bilancio_annuale_viaggi`.
   ⛔️ Finché ci sono, la chiamata interna di `fn_get_bilancio_viaggio_print_data` è **ambigua** e il
   database la rifiuta: **Stampa Bilancio Viaggio e Stampa Bilancio Annuale non funzionano**.
   Verificato in locale il 2026-09-17 — ed è molto probabile che in PROD sia lo stesso, perché le
   firme arrivano dagli stessi script.
2. Aggiunge `importo_effettivo_eur`, il valore su cui si calcola il margine: il lordo sui costi di
   chi non detrae l'IVA (forfettario), il netto in tutti gli altri casi.

⚠️ **Va insieme alla versione del programma che lo usa**: la stampa legge la colonna nuova. Script
...
> ### ✅ ELENCO ESTRATTO il 2026-09-08 — `Documents/Resoconti/2026-09-08-Clienti_da_Completare_prima_del_GoLive.md`
>
...
- `Documents/Versioni/Note_Rilascio_Versione_2_0.md`, Sezione 1 (funzionalità) e Sezione 2 (bug risolti,
...
- [ ] I due piani di test eseguiti: `Documents/Piani_Test/2026-08-20-Piano_Test_MAUI.md` e
      `Documents/Piani_Test/2026-08-20-Piano_Test_Flask.md`
...
- [ ] Solo **dopo** che tutto gira: eliminare le funzioni dell'elenco di ritiro
      (`Documents/Architettura/Funzioni_DB.md`, sezione «Elenco di ritiro»). Non prima: restano in piedi finché
...
- [x] **Cifratura reale segreti implementata** (§2.2) — fatta l'11/07/2026 (pgcrypto + GV_SECRET_KEY, commit f056469). ⚠️ Restano i passi di go-live: chiave in PROD e segreti re-inseriti.
...
- [x] **Sito di iscrizione ai viaggi rivisto (§2.8)** — fatto il 07/09/2026: consenso raccolto con data e fonte, avviso nome/sesso, titolo/sesso allineati, e i controlli del cliente ricondotti alle STESSE funzioni del gestionale. Collaudato: `Documents/Piani_Test/2026-09-07-Piano_Test_Sito_Iscrizione.md`.
...
- [ ] **Eseguito il Piano di Test del sito di iscrizione** (`Documents/Piani_Test/2026-09-07-Piano_Test_Sito_Iscrizione.md`) — nove gruppi, dall'apertura della pagina a cosa resta scritto a database. ⚠️ Il gruppo **I** è quello che verifica l'unificazione fra sito e gestionale: se un'iscrizione fatta dal sito non si riesce a risalvare dal gestionale, il sito ha scritto qualcosa che le regole non accettano.
...
- [ ] `Documents/Architettura/Funzioni_DB.md` allineato allo stato PROD.
...
for f in SqlScripts/*.sql; do
  ...
  grep -q "^| $n |" "Documents/Progetti/Estensione_Web/2026-07-10-Checklist_Go_Live_PROD.md" \
