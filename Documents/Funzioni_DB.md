...
| `fn_consenso_da_chiedere` | p_cliente_id integer | boolean | Se il consenso marketing va chiesto a questo cliente: solo a chi non ha mai risposto. |
...
| `fn_consenso_registra_risposta` | p_cliente_id integer, p_risposta boolean, p_fonte character varying DEFAULT 'iscrizione_web'::character varying | boolean | Registra la risposta alla richiesta di consenso, sì o no che sia. Il no non cambia |
...
| `fn_mov_clienti_viaggi_delete` | p_viaggio_id integer, p_data_viaggio_id integer, p_cliente_id integer | integer |  |
