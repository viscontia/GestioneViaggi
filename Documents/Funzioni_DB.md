...
**Convenzione CRUD web (valida per tutte le entità):** ogni funzione prende `p_azienda_id` ed è scoped su di esso (`WHERE ... AND azienda_id = p_azienda_id`) → isolamento multi-tenant totale, un'azienda non può leggere/modificare/eliminare righe di un'altra. `SECURITY INVOKER` (default, mai `SECURITY DEFINER`). Le colonne di audit (`created/created_by/updated/updated_by`) **non** sono mai passate: le valorizza il trigger `trg_web_audit()` dalla GUC `my.app_user`. `insert` ritorna il nuovo id; `update`/`delete` ritornano il numero di righe interessate (0/1).
