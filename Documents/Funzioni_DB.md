...
| `fn_web_tour_stato_sezioni` | p_contenuto_id bigint, p_azienda_id integer, p_lingue character varying[] | TABLE(ha_slug boolean, ha_sottotitolo boolean, ha_descrizione boolean, n_immagini integer, ha_principale boolean, n_giornate integer, n_traducibili integer, n_tradotte integer) | Fatti grezzi per il semaforo dei sotto-tab contenuti web di una edizione. Le soglie Vuoto/Parziale/Completo restano in C# (WebTabStatoRules). L'elenco dei campi traducibili deve restare allineato a WebTraduzioneOrchestratorService.GetTranslatableItemsAsync. |
