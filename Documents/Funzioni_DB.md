...
| `get_participants_sorted` | Restituisce partecipanti ordinati per equipaggio (Pilota + Passeggeri), inclusi dati anagrafici, residenza e documenti completi. | `p_data_viaggio_id integer` | `TABLE(viaggio_id integer, data_id integer, cliente_id integer, nominativo text, tipo_partecipante_id integer, ruolo text, note text, cane_sino varchar, intolleranze text, mezzo_dettagli text, cliente_pilota_id integer, grouping_key integer, is_pilot boolean, telefono text, email text, residenza text, codice_fiscale text, data_nascita date, luogo_nascita text, nazionalita text, tipo_documento text, numero_documento text, rilasciato_da text, data_rilascio date, data_scadenza date)` | `Services/CRUD/MovClientiViaggiService.cs`, `Services/Printing/ViaggiPrinter.cs` |
| `get_pilots_grouped_by_vehicle` | Restituisce SOLO i piloti con dati anagrafici completi, ordinati per marca e modello del mezzo. Usato per stampa scheda viaggio - sezione veicoli raggruppati. | `p_data_viaggio_id integer` | `TABLE(viaggio_id integer, data_id integer, cliente_id integer, nominativo text, marca text, modello text, targa text, telefono text, email text, residenza text, codice_fiscale text, data_nascita date, luogo_nascita text)` | `Services/Printing/TravelPrintService.cs`, `Services/Printing/ViaggiPrinter.cs` |
| `get_viaggi_grouped_by_year` | Restituisce viaggi e date viaggi raggruppati per anno con stato. Usato per TreeView in TravelDataSelectorDialog. Supporta filtro per azienda (0 o NULL per tutte - solo SuperAdmin). | `p_azienda_id integer` | `TABLE(anno integer, viaggio_id integer, viaggio_descrizione text, data_viaggio_id integer, data_inizio date, data_fine date, effettuato_sino char(1))` | `Services/CRUD/AnaViaggiService.cs`, `Components/Shared/TravelDataSelectorDialog.razor` |
...
| `get_rooming_list_data` | Restituisce tutti i partecipanti di un viaggio raggruppati per tipo camera con dati anagrafici completi, documenti e nazionalità per stampa Rooming List | `p_data_viaggio_id integer` | `TABLE(cliente_id integer, room_id integer, nominativo text, eta integer, data_nascita date, luogo_nascita text, indirizzo_residenza text, citta_residenza text, residenza_completa text, country_code text, country_name text, nationality text, tipo_documento text, numero_documento text, ente_rilascio text, data_rilascio date, data_scadenza date, intolleranze text, tipo_alloggio_id integer, tipo_alloggio_descrizione text, max_occupanti integer, sort_order integer)` | `Services/Printing/RoomingListPrintService.cs` |
