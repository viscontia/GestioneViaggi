...
→ *Raccomandazione:* per replicare la "scheda millimetrica" servono **due campi/liste dedicate** (incluso[], escluso[]) nel CMS (`web_tour_contenuti` o tabella figlia) + UI + traduzione. Piccola aggiunta, ma **da fare** se la vogliamo sul sito. In alternativa (rapida): convenzione redazionale dentro `altre_info_html` — meno pulita, non filtrabile/schematizzabile.
...
→ *Raccomandazione:* decidere la **fonte**: (1) **Google/TripAdvisor** (embed/API — social proof forte, meno lavoro CMS, ma vincoli di API/branding); (2) **gestione interna** (nuova tabella `web_recensioni` + moderazione + raccolta post-viaggio via email + privacy/consenso). IMTBike sbaglia mettendole in fondo: noi le posizioniamo **vicino alla CTA** nella scheda tour. Effort e privacy da valutare.
...
→ *Raccomandazione:* definire la capienza (max mezzi × posti, o campo esplicito su `ana_date_viaggi`) e un calcolo per i badge ("Ultimi posti", "Sold-out"). Piccola aggiunta dati + logica nello strato pubblico. Senza, i badge di disponibilità restano approssimativi.
...
→ *Raccomandazione:* decidere in fase di preventivo. Next.js/Nuxt = ecosistema SEO/performance maturo + Cloudflare; Blazor SSR = un solo stack .NET. Non promettere prima della scelta.
...
→ *Raccomandazione:* confermare in fase tecnica; incide su performance e costi.
...
→ *Raccomandazione:* nel doc cliente presentarlo come **opzionale, fase successiva**; non dare tempi. Grande valore ma grande effort.
...
→ *Raccomandazione:* concordare un **set fotografico curato** per i tour di punta e almeno 1 video hero; la vetrina "premium" vive o muore sul materiale visivo.
...
- [ ] Video hero: sì/no? chi produce il video? (fallback hero fotografico)
- [ ] Mappa interattiva nazione: dentro il 1° rilascio o fase 2? (effort dedicato)
- [ ] Incluso/Escluso: campi CMS dedicati (consigliato) o convenzione redazionale?
- [ ] Recensioni: Google/TripAdvisor vs interne? (fonte + privacy)
- [ ] Capienza/posti: definire il dato per i badge disponibilità
- [ ] Tour giornalieri/weekend: nuovo tipo viaggio in anagrafica? landing EN dedicate?
- [ ] Frontend: Blazor SSR vs Next/Nuxt (in preventivo, vincolo SEO)
- [ ] DB prod: Supabase vs Hetzner co-locato
- [ ] Pagamenti/fatturazione: confermare come fase successiva (no tempi ora)
- [ ] Set fotografico curato per i tour di punta
...
**Regola per il doc cliente:** presentare tutto ciò che il CMS **già alimenta** come "pronto"; presentare (b)(c)(d)(e) come parte del sito (senza allarmare, sono aggiunte naturali); tenere (h) pagamenti come opzionale/fase successiva.
