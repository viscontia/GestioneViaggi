...
L'Estensione Web + hardening introducono gli script **`SqlScripts/406` → `480`** (i numeri 445–449 non esistono; `475` = cifratura segreti; `476–480` = aggiunte CMS post-Blocco 13). Su un DB PROD che non li ha mai visti, il deploy = applicarli **tutti, in ordine numerico crescente**. Sono per la maggior parte idempotenti (function `CREATE OR REPLACE`, `IF NOT EXISTS`), ma **alcuni richiedono attenzione manuale** (vedi §2).
