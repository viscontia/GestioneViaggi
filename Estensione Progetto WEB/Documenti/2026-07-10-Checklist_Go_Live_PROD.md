...
```bash
for f in $(ls SqlScripts/*.sql | awk -F_ '$1>=406 && $1<=502' | sort -t_ -k1 -n); do
...
> Nota: questa tabella dettaglia i primi script; quelli `467`–`502` (Blocco 13, aggiunte CMS §A, integrità DB, semaforo sotto-tab, mappe multiple, gating traduzioni) sono descritti nei riquadri sopra. Il loop applica comunque **tutti** gli script 406–502 in ordine numerico.
...
- [ ] Applicati in ordine gli script 406–502 su PROD (§1) senza errori.
