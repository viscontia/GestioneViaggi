...
```bash
for f in $(ls SqlScripts/*.sql | awk -F_ '$1>=406 && $1<=508' | sort -t_ -k1 -n); do
...
> Nota: questa tabella dettaglia i primi script; quelli `467`–`508` (Blocco 13, aggiunte CMS §A, integrità DB, semaforo sotto-tab, mappe multiple, gating traduzioni) sono descritti nei riquadri sopra. Il loop applica comunque **tutti** gli script 406–508 in ordine numerico.
