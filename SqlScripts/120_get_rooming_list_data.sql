...
BEGIN
    ...
    WITH room_assignments AS (
        -- Appiattisci i partecipanti dalle 6 colonne della tabella mov_clienti_alloggi
        SELECT mca.mov_clienti_alloggio_pk AS room_id, mca.tipo_alloggio_id_fk, mca.cliente_id1_fk AS cliente_id FROM mov_clienti_alloggi mca WHERE mca.data_viaggio_id_fk = p_data_viaggio_id AND mca.cliente_id1_fk IS NOT NULL AND mca.cliente_id1_fk > 0
        UNION ALL
        SELECT mca.mov_clienti_alloggio_pk AS room_id, mca.tipo_alloggio_id_fk, mca.cliente_id2_fk AS cliente_id FROM mov_clienti_alloggi mca WHERE mca.data_viaggio_id_fk = p_data_viaggio_id AND mca.cliente_id2_fk IS NOT NULL AND mca.cliente_id2_fk > 0
        UNION ALL
        SELECT mca.mov_clienti_alloggio_pk AS room_id, mca.tipo_alloggio_id_fk, mca.cliente_id3_fk AS cliente_id FROM mov_clienti_alloggi mca WHERE mca.data_viaggio_id_fk = p_data_viaggio_id AND mca.cliente_id3_fk IS NOT NULL AND mca.cliente_id3_fk > 0
        UNION ALL
        SELECT mca.mov_clienti_alloggio_pk AS room_id, mca.tipo_alloggio_id_fk, mca.cliente_id4_fk AS cliente_id FROM mov_clienti_alloggi mca WHERE mca.data_viaggio_id_fk = p_data_viaggio_id AND mca.cliente_id4_fk IS NOT NULL AND mca.cliente_id4_fk > 0
        UNION ALL
        SELECT mca.mov_clienti_alloggio_pk AS room_id, mca.tipo_alloggio_id_fk, mca.cliente_id5_fk AS cliente_id FROM mov_clienti_alloggi mca WHERE mca.data_viaggio_id_fk = p_data_viaggio_id AND mca.cliente_id5_fk IS NOT NULL AND mca.cliente_id5_fk > 0
        UNION ALL
        SELECT mca.mov_clienti_alloggio_pk AS room_id, mca.tipo_alloggio_id_fk, mca.cliente_id6_fk AS cliente_id FROM mov_clienti_alloggi mca WHERE mca.data_viaggio_id_fk = p_data_viaggio_id AND mca.cliente_id6_fk IS NOT NULL AND mca.cliente_id6_fk > 0
    )
    SELECT
    ...
    ORDER BY
        COALESCE(ta.tipo_alloggio_id, 999999),  -- Camere assegnate prima, poi non assegnate
