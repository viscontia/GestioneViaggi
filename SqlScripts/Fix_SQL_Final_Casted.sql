-- ============================================================================
-- FIX FINALE ULTRA-COMPATIBILE: CAST ESPLICITI, AMBIGUITÀ E NOMI REALI
-- Autore: Antigravity
-- Data: 2026-02-12
-- ============================================================================
 ...
 BEGIN
     ...
     WITH filtered_data AS (
         SELECT 
             ...
             f.ragione_sociale as c_fornitore_rs_base,
             tc.causale_codice as c_tm_codice_base,
             tc.causale_descrizione as c_tm_desc_base,
             tc.causale_segno as c_segno_base,
             t.transazione_causale as c_causale_base,
             t.transazione_stato as c_stato_base,
             t.transazione_numero_documento as c_num_doc_base,
             v.valuta_codice_iso as c_v_iso_base,
             t.transazione_importo as c_importo_base,
             CASE 
             ...
             END as c_imp_target_base,
             vt.valuta_codice_iso as c_vt_iso_base,
             -- NOMI REALI DB: viaggio_descrizione_breve e data_viaggio_data_inizio
             av.viaggio_descrizione_breve as c_v_desc_base,
             adv.data_viaggio_data_inizio as c_v_data_inizio_base
     ...
     SELECT 
         (CASE 
             WHEN p_ordinamento IN ('FORNITORE', 'DATA_DOCUMENTO', 'DATA_DOCUMENTO_DESC') THEN 
                 CASE 
                     WHEN p_ordinamento = 'FORNITORE' THEN fd.c_fornitore_rs_base
                     ELSE TO_CHAR(fd.c_data_doc, 'YYYY-MM')
                 END
             WHEN p_ordinamento = 'TIPO_MOVIMENTO' THEN fd.c_tm_desc_base
             ELSE 'GENERALE'
         END)::text as g_chiave,
         (CASE 
             WHEN p_ordinamento IN ('FORNITORE', 'DATA_DOCUMENTO', 'DATA_DOCUMENTO_DESC') THEN 
                 CASE 
                     WHEN p_ordinamento = 'FORNITORE' THEN fd.c_fornitore_rs_base
         ...
         (CASE 
             ...
             ELSE 1
         END)::integer as g_ordine,
         fd.c_id::integer,
         fd.c_data::date,
         fd.c_data_doc::date,
         fd.c_data_scad::date,
         fd.c_data_pag::date,
         fd.c_fornitore_rs_base::text,
         fd.c_tm_codice_base::text,
         fd.c_tm_desc_base::text,
         fd.c_segno_base::integer,
         fd.c_causale_base::text,
         fd.c_stato_base::text,
         fd.c_num_doc_base::text,
         fd.c_v_iso_base::text,
         fd.c_importo_base::numeric,
         fd.c_imp_target_base::numeric,
         fd.c_vt_iso_base::text,
         fd.c_v_desc_base::text,
         fd.c_v_data_inizio_base::date
     FROM filtered_data fd
     ORDER BY 
         3 ASC, -- Posizione gruppo_ordine
         1 NULLS LAST, -- Posizione gruppo_chiave
         ...
         CASE WHEN p_ordinamento = 'IMPORTO_ASC' THEN fd.c_importo_base END ASC,
         CASE WHEN p_ordinamento = 'IMPORTO_DESC' THEN fd.c_importo_base END DESC,
         fd.c_id ASC;
 END;
 $function$;
