-- ============================================================================
-- CRITICAL FIX: Reload correct version of fn_get_transazioni_stampa_subtotali
--
-- PROBLEM: The database currently has an OLD version from update_print_functions.sql
--          that only returns 3 columns:
--          - gruppo_chiave
--          - totale_importo_algebrico
--          - conteggio_transazioni
--
-- SOLUTION: Drop all versions and load the CORRECT version that returns 11 columns
--           including totale_fatturato_target and totale_pagato_target
-- ============================================================================
...
-- Step 2: Create the CORRECT version with all 11 return columns
...
BEGIN
    ...
    IF v_return_type LIKE '%totale_fatturato_target%' AND v_return_type LIKE '%totale_pagato_target%' THEN
        RAISE NOTICE 'SUCCESS: Function has been correctly updated with all required columns';
    ELSE
        RAISE WARNING 'PROBLEM: Function may not have all required columns. Please check manually.';
    END IF;
END $$;
