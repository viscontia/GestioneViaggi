-- fn_web_tour_stato_sezioni: aggiunge le descrizioni delle mappe ai campi traducibili.
-- Segue gli script 492 (prima versione) e 493/494 (mappe multiple).
--
-- La descrizione di una mappa è testo mostrato al cliente, quindi entra fra i campi tradotti.
-- Va tenuta allineata a WebTraduzioneOrchestratorService.GetTranslatableItemsAsync: quella
-- costruisce l'elenco per il tab Traduzioni, questa conta lo stesso insieme per il semaforo.
-- Se divergono, il denominatore del conteggio è sbagliato e il tab non risulta mai completo.
...
'Fatti grezzi per il semaforo dei sotto-tab contenuti web di una edizione. Le soglie Vuoto/Parziale/Completo restano in C# (WebTabStatoRules). L''elenco dei campi traducibili deve restare allineato a WebTraduzioneOrchestratorService.GetTranslatableItemsAsync.';
