-- ========================================
-- Script: 476_AnaViaggi_InclusoEscluso.sql
-- "Incluso / Escluso" come attributo del VIAGGIO (uguale per tutte le edizioni),
-- come la difficoltà. Due campi HTML letti live dal sito (scheda tour). Tradotti
-- a livello viaggio (entita='ana_viaggi' in web_traduzioni). Deciso con Adriano:
-- niente tabella figlia strutturata.
-- ========================================

-- 1. Colonne (nullable, TEXT/HTML)
ALTER TABLE ana_viaggi ADD COLUMN IF NOT EXISTS viaggio_incluso TEXT;
ALTER TABLE ana_viaggi ADD COLUMN IF NOT EXISTS viaggio_escluso TEXT;

-- 2. sp_ana_viaggi_create (drop firma corrente + ricrea con incluso/escluso dopo difficolta)
DROP FUNCTION IF EXISTS sp_ana_viaggi_create(
    VARCHAR, TEXT, INTEGER, INTEGER, CHAR, INTEGER, VARCHAR, INTEGER, TEXT, VARCHAR,
...
BEGIN
    INSERT INTO ana_viaggi (
        ...
        viaggio_num_km, viaggio_difficolta, viaggio_incluso, viaggio_escluso, viaggio_tipo_avvicinamento_fk,
        ...
        p_viaggio_num_km, p_viaggio_difficolta, p_viaggio_incluso, p_viaggio_escluso, p_viaggio_tipo_avvicinamento_fk,
...
-- 3. sp_ana_viaggi_update (drop firma corrente + ricrea con incluso/escluso)
DROP FUNCTION IF EXISTS sp_ana_viaggi_update(
    INTEGER, VARCHAR, TEXT, INTEGER, INTEGER, CHAR, INTEGER, VARCHAR, INTEGER, TEXT, VARCHAR,
...
-- 4. fn_ana_viaggi_get_all (drop + ricrea con viaggio_incluso/escluso)
...
BEGIN
    ...
    SELECT
        ...
        v.viaggio_num_km, v.viaggio_difficolta, v.viaggio_incluso, v.viaggio_escluso, v.viaggio_note,
...
-- 5. fn_ana_viaggi_get_by_id (drop + ricrea con viaggio_incluso/escluso)
