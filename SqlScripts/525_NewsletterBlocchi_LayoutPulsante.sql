-- Posizione propria del pulsante dentro un blocco.
--
-- Sul blocco immagine il pulsante ereditava l'allineamento dell'IMMAGINE: con l'immagine a piena
-- larghezza finiva sempre a sinistra, senza modo di centrarlo o spostarlo. Sono due scelte
-- distinte e servono due campi distinti.
--
-- NULL = segue il layout del blocco, che e' il comportamento precedente: i blocchi esistenti non
-- cambiano aspetto.
--
-- Le firme precedenti vengono ESPLICITAMENTE rimosse: CREATE OR REPLACE non sostituisce una
-- function quando cambia il numero di parametri, e senza il DROP resterebbero versioni
-- sovrapposte che rendono ambigua ogni chiamata parziale (vedi script 524).

ALTER TABLE web_newsletter_blocchi ADD COLUMN IF NOT EXISTS layout_pulsante VARCHAR(10);

ALTER TABLE web_newsletter_blocchi DROP CONSTRAINT IF EXISTS chk_web_newsletter_blocchi_layout_pulsante;
ALTER TABLE web_newsletter_blocchi
    ADD CONSTRAINT chk_web_newsletter_blocchi_layout_pulsante
    CHECK (layout_pulsante IS NULL OR layout_pulsante IN ('sinistra','centro','destra'));

COMMENT ON COLUMN web_newsletter_blocchi.layout_pulsante IS
'Allineamento del pulsante, indipendente da quello dell''immagine. NULL = segue il layout del blocco.';
...
BEGIN
    ...
    INSERT INTO web_newsletter_blocchi(
        ...
        data_viaggio_id_fk, indirizzo_id_fk, social, icona_url, layout_pulsante, azienda_id)
    VALUES (
        ...
        NULLIF(btrim(COALESCE(p_layout_pulsante,'')), ''), p_azienda_id)
...
-- La clonazione porta con se' anche questo campo.
