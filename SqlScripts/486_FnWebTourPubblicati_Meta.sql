-- ============================================================================
-- Blocco 5 (Fase 2, Task 4) — SEO assistita: esporre meta_title/meta_description
-- su fn_web_tour_pubblicati con fallback DB-side.
--   * meta_title       = COALESCE(NULLIF(btrim(c.meta_title),''), b_titolo)
--   * meta_description = COALESCE(NULLIF(btrim(c.meta_description),''), b_sottotitolo)
-- Colonne aggiunte IN CODA al RETURNS TABLE (retro-compatibile per mapping per-nome).
-- Nota: meta non tradotti in questo pass (restano IT anche con p_lingua diversa) —
-- eventuale traduzione futura via web_traduzioni (entita 'web_tour_contenuti',
-- campi 'meta_title'/'meta_description').
-- ============================================================================
...
CREATE OR REPLACE FUNCTION fn_web_tour_pubblicati(p_azienda_id integer, p_lingua character DEFAULT 'IT'::bpchar)
 RETURNS TABLE(viaggio_id integer, contenuto_id bigint, titolo character varying, sottotitolo character varying,
               ...
               meta_title character varying, meta_description character varying)
 LANGUAGE sql
 STABLE
AS $function$
    WITH base AS (
