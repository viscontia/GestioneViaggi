-- ============================================================================
-- §A.3 Tour brevi / giornalieri.
-- Flag sul TIPO viaggio (ana_tipo_viaggi): marca i tipi "esperienza breve 1–3 gg".
-- Il sito userà il flag per una SEZIONE CONDIZIONALE "Tour giornalieri" che
-- compare SOLO se esistono tour pubblicati di quel tipo per l'azienda.
--   * fn_web_tour_pubblicati espone is_tour_breve per-edizione (per filtrare la sezione).
--   * fn_web_ha_tour_brevi_pubblicati dice se la sezione va mostrata (menu condizionale).
-- ============================================================================

BEGIN;

-- 1. Flag sul tipo viaggio
ALTER TABLE ana_tipo_viaggi ADD COLUMN IF NOT EXISTS tipo_viaggio_breve BOOLEAN NOT NULL DEFAULT false;

-- 2. Esiste almeno un tour BREVE pubblicato per l'azienda? (menu/sezione condizionale)
--    STABLE, EXECUTE ad anon: legge solo tabelle già esposte allo strato pubblico.
CREATE OR REPLACE FUNCTION fn_web_ha_tour_brevi_pubblicati(p_azienda_id integer)
...
$$;
GRANT EXECUTE ON FUNCTION fn_web_ha_tour_brevi_pubblicati(integer) TO anon;

-- 3. fn_web_tour_pubblicati: aggiungo is_tour_breve in coda (dal tipo del viaggio).
...
CREATE OR REPLACE FUNCTION fn_web_tour_pubblicati(p_azienda_id integer, p_lingua character DEFAULT 'IT'::bpchar)
 RETURNS TABLE(viaggio_id integer, contenuto_id bigint, titolo character varying, sottotitolo character varying,
               ...
               incluso text, escluso text, posti_rimasti integer, posti_stato text, is_tour_breve boolean)
 LANGUAGE sql STABLE
AS $function$
    WITH base AS (
